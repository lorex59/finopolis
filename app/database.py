...
# --- Override: append to existing selections instead of replacing -----------
def save_selected_positions(group_id: str, user_id: int, positions: list[dict]) -> None:
    """
    Append user's selected positions to existing ones. If a position with the same
    (group_id, user_id, name, price) exists, its quantity is increased. Otherwise, a new row is inserted.
    """
    conn = get_db_connection()
    cur = conn.cursor()
    for pos in positions or []:
        ...
        cur.execute(
            "SELECT id, quantity FROM selected_positions WHERE group_id = ? AND user_tg_id = ? AND name = ? AND price = ? ORDER BY id LIMIT 1",
            (str(group_id), str(user_id), name, price),
        )
        ex = cur.fetchone()
        if ex:
            new_q = float(ex['quantity']) + quantity
            cur.execute(
                "UPDATE selected_positions SET quantity = ? WHERE id = ?",
                (new_q, ex['id'])
            )
        else:
            cur.execute(
                "INSERT INTO selected_positions (group_id, user_tg_id, position_id, name, quantity, price) VALUES (?, ?, ?, ?, ?, ?)",
                (str(group_id), str(user_id), position_id, name, quantity, price)
            )
    conn.commit()
    conn.close()
    # sync in-memory
    _ = get_selected_positions(group_id)

def set_assignment(receipt_id: str, user_id: int, indices: list[int]) -> None:
    """
    Merge new indices with existing for the user.
    """
    existing = ASSIGNMENTS.get(receipt_id, defaultdict(list)).get(user_id, [])
    merged = list(existing) + list(indices or [])
    if receipt_id not in ASSIGNMENTS:
        ASSIGNMENTS[receipt_id] = defaultdict(list)
    ASSIGNMENTS[receipt_id][user_id] = merged

def calculate_group_balance(group_id: str) -> list[tuple[int, int, float]]:
    """
    Calculate minimal transfers (debtor, creditor, amount). Items where all users chose
    quantity==1 (ignoring zero) are split evenly: total_cost = original price * original qty,
    per-user share = total_cost / number_of_users_who_chose.
    Other items are summed as quantity*price per user.
    """
    positions = get_positions(group_id) or []
    pos_map = {}
    for p in positions:
        key = (p.get("name"), float(p.get("price",0)))
        pos_map[key] = float(p.get("quantity",1) or 1)
    selections = get_selected_positions(group_id) or {}
    # compute spend per user
    spend: dict[int, float] = {}
    # Build user selections keyed by (name, price) -> qty
    sel_by_key: dict[tuple[str,float], dict[int,float]] = {}
    for uid, items in selections.items():
        for it in items:
            key = (it.get("name"), float(it.get("price",0)))
            q = float(it.get("quantity", 1) or 1)
            sel_by_key.setdefault(key, {}).setdefault(uid, 0.0)
            sel_by_key[key][uid] += q
    for key, users_map in sel_by_key.items():
        name, price = key
        orig_qty = pos_map.get(key, 1.0)
        total_cost = price * orig_qty
        # decide even split: all positive quantities are exactly 1.0
        vals = [q for q in users_map.values() if q > 0]
        even = len(vals) >= 2 and all(abs(q - 1.0) < 1e-6 for q in vals)
        if even:
            share = total_cost / len(vals)
            for uid in users_map.keys():
                if users_map[uid] > 0:
                    spend[uid] = round(spend.get(uid, 0.0) + share, 2)
        else:
            for uid, q in users_map.items():
                spend[uid] = round(spend.get(uid, 0.0) + q * price, 2)
    # bring in payments (if implemented)
    # For now assume no pre-recorded payments; extend here if available.
    # Compute net balances: positive means they should receive, negative — pay
    total = sum(spend.values())
    # Normalize around mean if needed (not strictly necessary)
    # Build transfers using greedy
    debtors = [[uid, -amt] for uid, amt in spend.items() if amt < 0]  # not expected
    creditors = [[uid, amt] for uid, amt in spend.items() if amt > 0]
    # If there are no negative entries (only expenses), treat each user as debtor relative to average
    if not debtors and creditors:
        avg = total / max(1, len(spend))
        balances = {uid: spend.get(uid, 0.0) - avg for uid in spend}
