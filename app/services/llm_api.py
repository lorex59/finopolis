...
PROMPT = (
    "Ты — эксперт по распознаванию товарных чеков. "
    "Проанализируй изображение чека и извлеки список покупок. "
    "Верни строго **JSON-массив** объектов, каждый из которых содержит:\n"
    "- `name`: название товара (строка, без лишних символов и комментариев)\n"
    "- `quantity`: количество товара (число, даже если 1)\n"
    "- `price`: цена за единицу товара (число, без валютных символов)\n\n"
    "Правила:\n"
    "1. Не включай итоговые строки вроде 'ИТОГ', 'СУММА', 'СКИДКА', 'ОПЛАТА', 'СДАЧА'.\n"
    "2. Если указано '2х99.90', '2 x 99.90', '99.90*2' или подобное — количество = 2, цена = 99.90.\n"
    "3. Если указано '0.754 кг х 299.90', количество = 0.754, цена = 299.90.\n"
    "4. Игнорируй скидки, бонусы, акции, кассира, дату, магазин и прочее не относящееся к позициям.\n"
    "5. Если цена указана общей суммой (например '2 шт 199.80'), дели на количество, чтобы получить цену за единицу.\n"
    "6. Все числа — float, все строки — без кавычек внутри.\n"
    "7. Верни только JSON-массив без лишнего текста, примеров или пояснений.\n\n"
    "Пример ожидаемого результата:\n"
    "[\n"
    "  {\"name\": \"Молоко 2.5%\", \"quantity\": 1, \"price\": 89.9},\n"
    "  {\"name\": \"Яблоки\", \"quantity\": 1.254, \"price\": 129.9},\n"
    "  {\"name\": \"Печенье Oreo\", \"quantity\": 2, \"price\": 99.9}\n"
    "]"
)
